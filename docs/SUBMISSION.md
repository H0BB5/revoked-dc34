...
1. 0:00–0:30 — **Set the scene:** "Quick version: this is a kill switch for AI agents that can spend money. We call it Revoked. This is Claude, and I've given it a wallet so it can pay vendors and settle invoices — but not my keys; a signed, revocable permission: up to 10 CHEQ a transfer, testnet, nothing else. If it misbehaves, I revoke that permission — not on our server where we could quietly fudge it, but on a public blockchain anyone can check. And revoking takes hardware: the DEF CON badge you're wearing doubles as a security key (lift it). Watch a good day — then when it's compromised." Show both windows.
